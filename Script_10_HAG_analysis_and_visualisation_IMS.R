...
# Load data ----
dataset <- read_csv("processed_master_database.csv")  # Read in summary data.
...
dataset %>% 
    ...
    do({plot <- ggplot(., aes(x = HAG_plotmean_of_cellmax_m,
        ...
        ggsave(paste0("outputs/diagnostics/HAG_Vs_AGB/", "plot_", unique(.$ProjectCode), ".png", sep = ''), width = 10, height = 10, units = 'cm', plot = plot)
    })
