## Script for statisitcal anlaysis of photogrammetry data
## Andrew Cunliffe
## Started 2018-11-29

# Establish operating environment ----
# remove(list = ls())  # Clear all objects from envrionment.
# NB. This functionality is supersceeded by using R projects...
# rm(list=ls())
# setwd("C:/workspace/Geospatial_Pipeline/Geospatial_Pipeline_Python/")
setwd('..')  # Move working directory up once level. 
home <- getwd()
...
sev_dataset <- dataset[dataset$SevilletaIAV == TRUE, ]  # Observations from Sevilleta IAV (interannual variation).
...
# Hardcoded overrides for scaling specific plots
# max_agb <- 4000
# max_mean_hag <- 1


### Diagnostic: Mean versus median canopy height ####
    # create plot
...
### HAG Vs. AGB grouped by project (For diagnostic purposes) ###----
    ...
    for (i in unique(dataset$ProjectCode)){
           ...
           length(project_df$HAG_plotmean_of_cellmax_m[!is.na(project_df$HAG_plotmean_of_cellmax_m)])> 0)
        {
            # Unconstrained intercepts
                ...
                outfile <- file.path(home, "outputs/Diagnostics/HAG_Vs_AGB by Project Unconstrained", paste("MeanHAG_Vs_AGB ",i,".png",sep=""))
                png(filename=outfile, width = 10, height = 10, units = 'cm', res = 400)
                plot(p)
                dev.off()
            ...
            # Constrained intercepts
                p <- ggplot(data = project_df, 
                    ...
                    theme(legend.position = c(0.35, 0.8)) +
                    coord_cartesian(ylim = c(0, max_agb), xlim = c(min_mean_hag, max_mean_hag), expand=FALSE) +
                    geom_smooth(method="lmrob", formula= y ~ x-1, aes(group=binomial_species), se=TRUE, size=0.5, na.rm = TRUE)
                
                outfile <- file.path(home, "outputs/Diagnostics/HAG_Vs_AGB by Project Constrained", paste("MeanHAG_Vs_AGB ",i,".png",sep=""))
                png(filename=outfile, width = 10, height = 10, units = 'cm', res = 400)
                plot(p)
                dev.off()
        }
...
# Initialize empty list for multipanel plot.
...
for (i in sort(unique(peakdataset$binomial_species))){
    ...
    x <- taxa_df$HAG_plotmean_of_cellmax_m
    y <- taxa_df$AGB_spatially_normalised_g_m2
    
    #  check if both x and y observations exist for this taxa, and exclude 'Mixed Grasses' taxon from plottings.
    ...
    if(valid_observations && i != 'Mixed Grasses'){
        ...
        # Save species-level plots
            outfile <- file.path(home, "outputs/HAG_Vs_AGB_by_Taxa",paste("MeanHAG_Vs_AGB ",i,".png",sep=""))
            png(filename=outfile, width = 10, height = 10, units = 'cm', res = 400)
...
#### Figure S1 ####
    ...
    # Tall
        ...
        outfile <- file.path(home, "outputs",paste("Figure S1 MeanHAG Vs AGB by taxa.png",sep=""))
    ...
    # Wide
        ...
        outfile <- file.path(home, "outputs",paste("Figure S1 MeanHAG Vs AGB by taxa - wide.png",sep=""))
...
for (i in sort(unique(peakdataset$plant_functional_type))){
    ...
    if(valid_observations){
        ...
        outfile <- file.path(home, "outputs/HAG_Vs_AGB_by_PFT",paste("MeanHAG Vs AGB ",i,".png",sep=""))
...
outfile <- file.path(home, "outputs",paste("MeanHAG_Vs_AGB by PFT.png",sep=""))
...
# ceate new dataframe for plotting
model_summary <- pft_summary_table
model_summary$plant_functional_type <- factor(model_summary$plant_functional_type, levels = unique(model_summary$plant_functional_type))
model_summary$slope_min <- model_summary$slope - model_summary$slope_error
model_summary$slope_max <- model_summary$slope + model_summary$slope_error
...
#### Main scatter plot ####
    ...
    # Create plot of r2 values. 
    pft_model_r2 <- ggplot(data = model_summary,
                           aes(x = model_summary$plant_functional_group, y = model_summary$r2))+
        geom_point(aes(shape=model_summary$plant_functional_type, col= model_summary$plant_functional_type), size=2) +
        coord_cartesian(ylim = c(0, 1)) +
        theme_models() +
        theme(legend.position = "none") +
    ...
    # Create plot of model slopes
    pft_model_slope <- ggplot(data = model_summary,
                              aes(x = model_summary$plant_functional_type, y = model_summary$slope))+
        geom_point(aes(shape=model_summary$plant_functional_type, col= model_summary$plant_functional_type), size=2) +
        geom_linerange(aes(x = model_summary$plant_functional_type, ymin = model_summary$slope_min, ymax = model_summary$slope_max, col= model_summary$plant_functional_type)) +
        scale_y_continuous(limits = c(1, (1.15*max(model_summary$slope))), breaks = scales::pretty_breaks(n = 5)) +
        theme_models() +
        scale_colour_viridis_d() +
        scale_shape_manual(values = pft_shapes) +
        grids(axis = "x", col="lightgrey", size=0.4, linetype = "dashed") +
...
#### Figure 3 ####
## Plot sampled climate space
    # Standard plot (not used) ----
        # # create plot
        # sampled_climate_space <- ggplot(data = peakdataset,
        #                                 aes(x = MAT,
        #                                     y = MAP)) +
        #     geom_point() +
        ...
        #     theme_coding() +
        #     coord_cartesian(xlim = c(mat_min, mat_max), ylim = c(0, map_max), expand=FALSE)
        # 
        # # Save plot
...
# Mean HAG Vs. AGB grouped by species
    ...
    for (i in sort(unique(sev_dataset$binomial_species))){
        ...
        if(valid_observations){
            ...
            outfile <- file.path(home, "outputs/Sevilleta/",paste("MeanHAG_Vs_AGB ",i,".png",sep=""))
            png(filename=outfile, width = 10, height = 10, units = 'cm', res = 400)
            plot(meanHAG_vs_AGB_species)
            dev.off()
